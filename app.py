...
def _adapt_sql(sql, params=()):
    ...
    sql = re.sub(r"date\((?:\"|' )?now(?:\"|' )?\)".replace(' ', ''), 'CURRENT_DATE', sql, flags=re.IGNORECASE)
...
def module_income_share(finance):
    finance = finance or {}
    profit = max(float(finance.get('profit') or 0), 0.0)
    return round(profit * 0.5, 2)


def share_basis_from_sales(finance):
    return module_income_share(finance)


def participant_share_breakdown(module_name, cycle=None, finance=None):
    participants = participant_rows_for_profit(module_name, cycle)
    income_share = module_income_share(finance)
    if not participants:
        return {
            'participants': [],
            'income_share': income_share,
            'owner_share_total': income_share,
            'caretaker_share_total': income_share,
        }

    owners = [p for p in participants if (p.get('role') or '').strip().lower() == 'owner']
    caretakers = [p for p in participants if (p.get('role') or '').strip().lower() == 'caretaker']

    display_rows = []
    if owners or caretakers:
        owner_each = (income_share / len(owners)) if owners else 0.0
        caretaker_each = (income_share / len(caretakers)) if caretakers else 0.0
        for p in participants:
            item = dict(p)
            role = (item.get('role') or '').strip().lower()
            if role == 'owner':
                item['income_share'] = round(owner_each, 2)
            elif role == 'caretaker':
                item['income_share'] = round(caretaker_each, 2)
            else:
                item['income_share'] = 0.0
            display_rows.append(item)
    else:
        share_each = (income_share / len(participants)) if participants else 0.0
        for p in participants:
            item = dict(p)
            item['income_share'] = round(share_each, 2)
            display_rows.append(item)

    return {
        'participants': display_rows,
        'income_share': round(income_share, 2),
        'owner_share_total': round(income_share if owners else 0.0, 2),
        'caretaker_share_total': round(income_share if caretakers else 0.0, 2),
    }
...
def participant_allocations(cycle, finance):
    if not cycle:
        return []
    module_name = cycle['module_name'] if cycle and hasattr(cycle, 'keys') and 'module_name' in cycle.keys() else ''
    breakdown = participant_share_breakdown(module_name, cycle, finance)
    allocations = []
    for p in breakdown['participants']:
        allocations.append({
            'id': p['id'],
            'name': p['name'],
            'role': p['role'],
            'profit': round(float(finance.get('profit') or 0), 2),
            'income_share': round(float(p.get('income_share') or 0), 2),
            'share': round(float(p.get('income_share') or 0), 2),
...
def participant_display_rows(module_name, cycle, finance):
    breakdown = participant_share_breakdown(module_name, cycle, finance)
    return breakdown['participants'], breakdown['income_share']
...
def poultry_house_monitoring():
    ...
    for row in houses.values():
        ...
        row['income_share'] = module_income_share(row)
...
def dashboard_context():
    ...
    hog_rows = query('SELECT hog_type, heads FROM hog_cycles WHERE status="ACTIVE"')
    hog_sold = float(query('SELECT COALESCE(SUM(heads),0) v FROM hog_sales', one=True)['v'] or 0)
    hog_breakdown = {'piglets': 0, 'sows': 0, 'fattener': 0, 'boar': 0, 'other': 0}
    for row in hog_rows:
        hog_type = clean_text(row.get('hog_type') if hasattr(row, 'get') else row['hog_type']).lower()
        heads = float(row['heads'] or 0)
        if 'piglet' in hog_type:
            hog_breakdown['piglets'] += heads
        elif 'sow' in hog_type or 'breeder' in hog_type:
            hog_breakdown['sows'] += heads
        elif 'fattener' in hog_type or 'finisher' in hog_type or 'grower' in hog_type:
            hog_breakdown['fattener'] += heads
        elif 'boar' in hog_type:
            hog_breakdown['boar'] += heads
        else:
    ...
    income_share_chart = {k.lower(): module_income_share(v) for k, v in modules.items()}
    capital_chart = dict(income_share_chart)
    ...
    return {
        ...
        'income_share_chart': income_share_chart,
        ...
        'bank_accounts': query('SELECT * FROM bank_accounts ORDER BY account_name, bank_name, id'),
...
def bank_option_label(account):
    ...
    details = ' · '.join(x for x in [bank_name, account_type] if x)
    return f"{account_name} — {details}" if details else account_name
...
def sync_bank_balance(account_id, amount, module_name, entry_date, tx_type, reference_no='', purpose='', notes=''):
    ...
    if not account:
        return False
    current_balance = safe_float(account.get('current_balance'))
    new_balance = current_balance + amount if tx_type in ('DEPOSIT', 'TRANSFER IN') else current_balance - amount
...
def migrate_db():
    ...
    if USE_POSTGRES:
        ...
        with db:
            with db.cursor() as cur:
                cur.execute("UPDATE participants SET module_name='POULTRY' WHERE module_name IS NULL OR TRIM(module_name)=''")
                cur.execute('''
                    CREATE TABLE IF NOT EXISTS owner_withdrawals(
                        id SERIAL PRIMARY KEY,
                        entry_date TEXT NOT NULL,
    ...
    ensure_column('poultry_sales', 'account_id', 'INTEGER')
    ensure_column('poultry_expenses', 'account_id', 'INTEGER')
    ensure_column('hog_sales', 'account_id', 'INTEGER')
    ensure_column('hog_expenses', 'account_id', 'INTEGER')
    ensure_column('fish_transactions', 'account_id', 'INTEGER')
    ensure_column('fish_expenses', 'account_id', 'INTEGER')

    db = _connect_db()
    sql = "UPDATE hog_cycles SET hog_type=CASE WHEN LOWER(COALESCE(pen_name,'')) LIKE '%piglet%' THEN 'Piglets' WHEN LOWER(COALESCE(pen_name,'')) LIKE '%sow%' OR LOWER(COALESCE(pen_name,'')) LIKE '%breeder%' THEN 'Sows' WHEN LOWER(COALESCE(pen_name,'')) LIKE '%fattener%' OR LOWER(COALESCE(pen_name,'')) LIKE '%finisher%' OR LOWER(COALESCE(pen_name,'')) LIKE '%grower%' THEN 'Fattener' WHEN LOWER(COALESCE(pen_name,'')) LIKE '%boar%' THEN 'Boar' ELSE 'Other' END WHERE hog_type IS NULL OR TRIM(hog_type)=''"
    sql, params = _adapt_sql(sql, ())
    if USE_POSTGRES:
        with db.cursor() as cur:
            cur.execute(sql, params)
        db.commit()
...
def owner_withdraw():
    ...
    execute('UPDATE bank_accounts SET current_balance=? WHERE id=?', (current_balance - amount, account_id))
    execute(
        'INSERT INTO bank_transactions(entry_date,account_id,module_name,tx_type,amount,reference_no,purpose,notes) VALUES(?,?,?,?,?,?,?,?)',
        (entry_date, account_id, 'OWNER', 'WITHDRAWAL', amount, reference_no, 'Owner profit withdrawal', notes)
    )
...
@login_required
def capital_add():
    module_name = clean_text(request.form.get('module_name')).upper()
    if not can_access_module(module_name):
        flash('You do not have access to add capital to that module.', 'danger')
        return redirect(url_for('poultry_page'))
    entry_date = safe_date(request.form.get('entry_date'), str(date.today()))
    amount = safe_float(request.form.get('amount'))
    destination_account_id = request.form.get('destination_account_id') or None
    capital_id = execute('INSERT INTO capital_entries(cycle_id,module_name,entry_date,source_name,amount,destination_account_id,notes) VALUES(?,?,?,?,?,?,?)',
            (request.form.get('cycle_id') or None, module_name, entry_date, clean_text(request.form.get('source_name')), amount, destination_account_id, clean_text(request.form.get('notes'))))
    if destination_account_id:
        sync_bank_balance(destination_account_id, amount, module_name, entry_date, 'DEPOSIT', purpose=f'{module_name} capital', notes=clean_text(request.form.get('notes')))
    log_audit('ADD', 'capital', capital_id or 0, f"{module_name} capital {amount}")
    ...
    if request.method == 'POST':
        ...
        if form_name == 'sale':
            ...
            execute('INSERT INTO poultry_sales(batch_id,entry_date,buyer,birds_sold,kilos,price_per_kilo,total_amount,notes,account_id) VALUES(?,?,?,?,?,?,?,?,?)',
                    (int(request.form['batch_id']), entry_date, clean_text(request.form.get('buyer')), safe_int(request.form.get('birds_sold')), kilos, ppk, total, clean_text(request.form.get('notes')), request.form.get('account_id') or None))
            if request.form.get('account_id'):
                sync_bank_balance(request.form.get('account_id'), total, 'POULTRY', entry_date, 'DEPOSIT', purpose='Poultry sale', notes=clean_text(request.form.get('notes')))
        ...
        if form_name == 'expense':
            ...
            exp_id = execute(
                ...
                (
                    ...
                    amount,
                    f"{clean_text(request.form.get('item_name'))}{' - ' + clean_text(request.form.get('description')) if request.form.get('description') else ''}",
                    receipt_file,
                    request.form.get('account_id') or None
                )
            )
            if request.form.get('account_id'):
                sync_bank_balance(request.form.get('account_id'), amount, 'POULTRY', entry_date, 'WITHDRAWAL', purpose=clean_text(request.form.get('category'), 'Poultry expense'), notes=clean_text(request.form.get('description')))
        ...
        if form_name == 'bulk_expense':
            ...
            for item in rows:
                ...
                execute('INSERT INTO poultry_expenses(batch_id,entry_date,category,amount,description,group_ref,receipt_file,account_id) VALUES(?,?,?,?,?,?,?,?)', (int(request.form['batch_id']), entry_date, item['category'], item['amount'], desc, group_ref, receipt_file, request.form.get('account_id') or None))
            if request.form.get('account_id'):
                sync_bank_balance(request.form.get('account_id'), sum(float(item['amount'] or 0) for item in rows), 'POULTRY', entry_date, 'WITHDRAWAL', purpose='Poultry bulk expense', notes=group_ref)
...
def hog_page():
    ...
    if request.method == 'POST':
        ...
        if form_name == 'hog_cycle':
            execute('INSERT INTO hog_cycles(cycle_id,pen_name,start_date,heads,source,cost,notes,hog_type) VALUES(?,?,?,?,?,?,?,?)',
                    (cycle['id'] if cycle else None, clean_text(request.form.get('pen_name')), safe_date(request.form.get('start_date'), str(date.today())), safe_int(request.form.get('heads')), clean_text(request.form.get('source')), safe_float(request.form.get('cost')), clean_text(request.form.get('notes')), clean_text(request.form.get('hog_type'), 'Other')))
        ...
        if form_name == 'hog_sale':
            ...
            execute('INSERT INTO hog_sales(hog_cycle_id,entry_date,buyer,heads,kilos,total_amount,notes,account_id) VALUES(?,?,?,?,?,?,?,?)',
                    (hog_cycle_id, entry_date, clean_text(request.form.get('buyer')), safe_int(request.form.get('heads')), safe_float(request.form.get('kilos')), total_amount, clean_text(request.form.get('notes')), request.form.get('account_id') or None))
            if request.form.get('account_id'):
                sync_bank_balance(request.form.get('account_id'), total_amount, 'HOG', entry_date, 'DEPOSIT', purpose='Hog sale', notes=clean_text(request.form.get('notes')))
        ...
        if form_name == 'hog_expense':
            ...
            execute('INSERT INTO hog_expenses(hog_cycle_id,entry_date,category,amount,description,receipt_file,receipt_name,account_id) VALUES(?,?,?,?,?,?,?,?)',
                    (hog_cycle_id, entry_date, clean_text(request.form.get('category'),'Other'), amount, clean_text(request.form.get('description')), receipt_file, secure_filename(getattr(request.files.get('receipt_file'), 'filename', '') or ''), request.form.get('account_id') or None))
            if request.form.get('account_id'):
                sync_bank_balance(request.form.get('account_id'), amount, 'HOG', entry_date, 'WITHDRAWAL', purpose=clean_text(request.form.get('category'), 'Hog expense'), notes=clean_text(request.form.get('description')))
        ...
        if form_name == 'hog_bulk_expense':
            ...
            if request.form.get('account_id'):
                sync_bank_balance(request.form.get('account_id'), sum(float(item['amount'] or 0) for item in rows), 'HOG', entry_date, 'WITHDRAWAL', purpose='Hog bulk expense', notes=group_ref)
    ...
    return render_template('hog.html', tab=tab, cycle=cycle, cycle_history=cycle_history('HOG'), cycles=cycles, feed_logs=feed_logs, sales=sales, expenses=expenses, capital=capital, finance=finance, finance_history=finance_history, participants=available_participants, available_participants=available_participants, share=share, share_participants=share_participants, expense_categories=EXPENSE_CATEGORIES, bank_accounts=query('SELECT * FROM bank_accounts ORDER BY account_name, bank_name, id'), cashflow_rows=cashflow_rows, cashflow_summary=cashflow_summary, visuals=visuals)
    ...
    if request.method == 'POST':
        ...
        if form_name == 'fish_tx':
            ...
            if not fc_id and cycle:
                fc_id = execute('INSERT INTO fish_cycles(cycle_id,period_name,start_date,status,notes) VALUES(?,?,?,?,?)', (cycle['id'], clean_text(request.form.get('period_name'), cycle['cycle_name']), safe_date(request.form.get('entry_date')), 'ACTIVE', clean_text(request.form.get('notes'))))
            entry_date = safe_date(request.form.get('entry_date'), str(date.today()))
            transaction_type = clean_text(request.form.get('transaction_type'),'BUY').upper()
            execute('INSERT INTO fish_transactions(fish_cycle_id,entry_date,transaction_type,supplier,buyer,species,kilos,price_per_kilo,total_amount,notes,account_id) VALUES(?,?,?,?,?,?,?,?,?,?,?)',
                    (fc_id, entry_date, transaction_type, clean_text(request.form.get('supplier')), clean_text(request.form.get('buyer')), clean_text(request.form.get('species'),'Fish'), kilos, ppk, total, clean_text(request.form.get('notes')), request.form.get('account_id') or None))
            if request.form.get('account_id') and total > 0:
                sync_bank_balance(request.form.get('account_id'), total, 'FISH', entry_date, 'WITHDRAWAL' if transaction_type == 'BUY' else 'DEPOSIT', purpose=f'Fish {transaction_type.lower()}', notes=clean_text(request.form.get('notes')))
        ...
        if form_name == 'fish_expense':
            ...
            execute('INSERT INTO fish_expenses(fish_cycle_id,entry_date,category,amount,description,receipt_file,receipt_name,account_id) VALUES(?,?,?,?,?,?,?,?)',
                    (fc_id, entry_date, clean_text(request.form.get('category'),'Other'), amount, clean_text(request.form.get('description')), receipt_file, secure_filename(getattr(request.files.get('receipt_file'), 'filename', '') or ''), request.form.get('account_id') or None))
            if request.form.get('account_id'):
                sync_bank_balance(request.form.get('account_id'), amount, 'FISH', entry_date, 'WITHDRAWAL', purpose=clean_text(request.form.get('category'), 'Fish expense'), notes=clean_text(request.form.get('description')))
        ...
        if form_name == 'fish_bulk_expense':
            ...
            if request.form.get('account_id'):
                sync_bank_balance(request.form.get('account_id'), sum(float(item['amount'] or 0) for item in rows), 'FISH', entry_date, 'WITHDRAWAL', purpose='Fish bulk expense', notes=group_ref)
...
def get_edit_config(record_type):
    configs = {
        ...
        'hog_cycle': {'table':'hog_cycles','fields':['pen_name','hog_type','start_date','heads','source','cost','status','notes'],'redirect':'hog_page','tab':'operations'},
...
def record_edit(record_type, record_id):
    ...
    return render_template('record_edit.html', record=row, record_type=record_type, fields=cfg['fields'], expense_categories=EXPENSE_CATEGORIES, tx_types=TX_TYPES, hog_types=['Piglets','Sows','Fattener','Boar','Other'])
