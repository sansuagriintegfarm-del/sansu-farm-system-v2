...
CLOUDINARY_FOLDER = os.getenv('CLOUDINARY_FOLDER', 'sansu_receipts').strip() or 'sansu_receipts'
...
def cloudinary_download_url(url):
    if not url:
        return ''
    return url.replace('/upload/', '/upload/fl_attachment/') if '/upload/' in url else url
...
def latest_poultry_batch_id(cycle=None):
    if cycle:
        row = query('SELECT id FROM poultry_batches WHERE cycle_id=? ORDER BY id DESC LIMIT 1', (cycle['id'],), one=True)
        if row:
            return row['id']
    row = query('SELECT id FROM poultry_batches ORDER BY id DESC LIMIT 1', one=True)
    return row['id'] if row else None


def latest_hog_cycle_id(cycle=None):
    if cycle:
        row = query('SELECT id FROM hog_cycles WHERE cycle_id=? ORDER BY id DESC LIMIT 1', (cycle['id'],), one=True)
        if row:
            return row['id']
    row = query('SELECT id FROM hog_cycles ORDER BY id DESC LIMIT 1', one=True)
    return row['id'] if row else None


def latest_fish_cycle_id(cycle=None):
    if cycle:
        row = query('SELECT id FROM fish_cycles WHERE cycle_id=? ORDER BY id DESC LIMIT 1', (cycle['id'],), one=True)
        if row:
            return row['id']
    row = query('SELECT id FROM fish_cycles ORDER BY id DESC LIMIT 1', one=True)
    return row['id'] if row else None
...
def migrate_db():
    ...
    ensure_column('poultry_expenses', 'receipt_name', 'TEXT')
...
def inject_globals():
    ...
    return {
        ...
        'fmt_money': fmt_money,
        'receipt_preview_url': lambda receipt: f"/receipts/{receipt}" if receipt else '',
        'receipt_download_url': lambda receipt: f"/receipts-download/{receipt}" if receipt else '',
...
@app.route('/receipts-download/<path:filename>')
@login_required
def download_receipt(filename):
    if filename.startswith('http://') or filename.startswith('https://'):
        return redirect(cloudinary_download_url(filename))
...
def capital_add():
    ...
    execute('INSERT INTO capital_entries(cycle_id,module_name,entry_date,source_name,amount,destination_account_id,notes) VALUES(?,?,?,?,?,?,?)',
            (request.form.get('cycle_id') or None, request.form['module_name'], safe_date(request.form.get('entry_date')), clean_text(request.form.get('source_name')), safe_float(request.form.get('amount')), request.form.get('destination_account_id') or None, clean_text(request.form.get('notes'))))
...
def relink_legacy_records():
    cycle = get_cycle('POULTRY')
    latest_batch = latest_poultry_batch_id(cycle)
    if latest_batch:
        execute('UPDATE poultry_expenses SET batch_id=? WHERE batch_id IS NULL', (latest_batch,))
        execute('UPDATE poultry_sales SET batch_id=? WHERE batch_id IS NULL', (latest_batch,))
...
def poultry_page():
    ...
    cycle = get_cycle('POULTRY')
    relink_legacy_records()
    if request.method == 'POST':
        form_name = request.form.get('form_name')
        if form_name == 'batch':
            batch_name = clean_text(request.form.get('batch_name'), f"{clean_text(request.form.get('poultry_type'), 'Poultry')} Batch {today_iso()}")
            ...
            execute('INSERT INTO poultry_batches(cycle_id,poultry_type,batch_name,house_name,start_date,birds_count,supplier,cost,notes) VALUES(?,?,?,?,?,?,?,?,?)',
                    (cycle['id'] if cycle else None, clean_text(request.form.get('poultry_type'), 'Broiler'), batch_name, clean_text(request.form.get('house_name')), start_date, safe_int(request.form.get('birds_count')), clean_text(request.form.get('supplier')), safe_float(request.form.get('cost')), clean_text(request.form.get('notes'))))
            flash('Batch added.', 'success')
            return redirect(url_for('poultry_page', tab='batches'))
        if form_name == 'mortality':
            batch_id = safe_int(request.form.get('batch_id')) or latest_poultry_batch_id(cycle)
            if not batch_id:
                flash('Create a batch first before saving mortality.', 'danger')
                return redirect(url_for('poultry_page', tab='operations'))
            execute('INSERT INTO poultry_mortality(batch_id,entry_date,deaths,notes) VALUES(?,?,?,?)', (batch_id, safe_date(request.form.get('entry_date')), safe_int(request.form.get('deaths')), clean_text(request.form.get('notes'))))
            flash('Mortality saved.', 'success')
            return redirect(url_for('poultry_page', tab='operations'))
        if form_name == 'feed':
            batch_id = safe_int(request.form.get('batch_id')) or latest_poultry_batch_id(cycle)
            if not batch_id:
                flash('Create a batch first before saving feed usage.', 'danger')
                return redirect(url_for('poultry_page', tab='operations'))
            feed_id = execute('INSERT INTO poultry_feed_logs(batch_id,entry_date,feed_type,bags,amount,notes) VALUES(?,?,?,?,?,?)', (batch_id, safe_date(request.form.get('entry_date')), clean_text(request.form.get('feed_type'), 'Feed'), safe_float(request.form.get('bags')), safe_float(request.form.get('amount')), clean_text(request.form.get('notes'))))
        ...
        if form_name == 'feed_stock':
            sacks = safe_float(request.form.get('sacks'))
            cps = safe_float(request.form.get('cost_per_sack'))
            total = sacks * cps
            inv_id = execute('INSERT INTO feed_inventory(module_name,entry_date,feed_type,sacks,cost_per_sack,total_cost,source_name,usage_type,house_name,cycle_id,notes) VALUES(?,?,?,?,?,?,?,?,?,?,?)', ('POULTRY', safe_date(request.form.get('entry_date')), clean_text(request.form.get('feed_type'), 'Feed'), sacks, cps, total, clean_text(request.form.get('source_name')), clean_text(request.form.get('usage_type'), 'Add to Inventory'), clean_text(request.form.get('house_name')), cycle['id'] if cycle else None, clean_text(request.form.get('notes'))))
        ...
        if form_name == 'sale':
            batch_id = safe_int(request.form.get('batch_id')) or latest_poultry_batch_id(cycle)
            if not batch_id:
                flash('Create a batch first before saving a sale.', 'danger')
            ...
            total = safe_float(request.form.get('total_amount')) or (kilos * ppk)
            execute('INSERT INTO poultry_sales(batch_id,entry_date,buyer,birds_sold,kilos,price_per_kilo,total_amount,notes) VALUES(?,?,?,?,?,?,?,?)',
                    (batch_id, safe_date(request.form.get('entry_date')), clean_text(request.form.get('buyer')), safe_int(request.form.get('birds_sold')), kilos, ppk, total, clean_text(request.form.get('notes'))))
            flash('Poultry sale saved.', 'success')
            return redirect(url_for('poultry_page', tab='finance'))
        if form_name == 'expense':
            batch_id = safe_int(request.form.get('batch_id')) or latest_poultry_batch_id(cycle)
            receipt_file = save_uploaded_receipt(request.files.get('receipt_file'), f"poultry_{safe_date(request.form.get('entry_date'))}")
            item_name = clean_text(request.form.get('item_name'), clean_text(request.form.get('category'), 'Expense'))
            description = clean_text(request.form.get('description'))
            details = item_name if not description else f"{item_name} - {description}"
            exp_id = execute('INSERT INTO poultry_expenses(batch_id,entry_date,category,amount,description,receipt_file,receipt_name) VALUES(?,?,?,?,?,?,?)',
                (batch_id, safe_date(request.form.get('entry_date')), clean_text(request.form.get('category'), 'Other'), safe_float(request.form.get('amount')), details, receipt_file, secure_filename(getattr(request.files.get('receipt_file'), 'filename', '') or '')))
            log_audit('ADD', 'poultry_expense', exp_id, details)
            flash('Expense saved.', 'success')
            return redirect(url_for('poultry_page', tab='finance'))
        if form_name == 'bulk_expense':
            batch_id = safe_int(request.form.get('batch_id')) or latest_poultry_batch_id(cycle)
            if not batch_id:
                flash('Create a batch first before saving expenses.', 'danger')
                return redirect(url_for('poultry_page', tab='finance'))
            ...
            group_ref = f"PB-{entry_date}-{int(batch_id or 0)}-{len(rows)}"
            ...
            for item in rows:
                ...
                execute('INSERT INTO poultry_expenses(batch_id,entry_date,category,amount,description,group_ref,receipt_file,receipt_name) VALUES(?,?,?,?,?,?,?,?)', (batch_id, entry_date, item['category'], item['amount'], desc, group_ref, receipt_file, receipt_name))
    ...
    if request.method == 'POST':
        ...
        if form_name == 'hog_cycle':
            execute('INSERT INTO hog_cycles(cycle_id,pen_name,start_date,heads,source,cost,notes) VALUES(?,?,?,?,?,?,?)',
                    (cycle['id'] if cycle else None, clean_text(request.form.get('pen_name'), f'Hog Pen {today_iso()}'), safe_date(request.form.get('start_date')), safe_int(request.form.get('heads')), clean_text(request.form.get('source')), safe_float(request.form.get('cost')), clean_text(request.form.get('notes'))))
        ...
        if form_name == 'hog_feed':
            execute('INSERT INTO hog_feed_logs(hog_cycle_id,entry_date,feed_type,quantity,amount,notes) VALUES(?,?,?,?,?,?)',
                    (int(request.form['hog_cycle_id']), request.form.get('entry_date'), request.form.get('feed_type'), request.form.get('quantity'), float(request.form.get('amount') or 0), request.form.get('notes')))
        ...
        if form_name == 'hog_sale':
            execute('INSERT INTO hog_sales(hog_cycle_id,entry_date,buyer,heads,kilos,total_amount,notes) VALUES(?,?,?,?,?,?,?)',
                    (int(request.form['hog_cycle_id']), request.form.get('entry_date'), request.form.get('buyer'), int(request.form.get('heads') or 0), float(request.form.get('kilos') or 0), float(request.form.get('total_amount') or 0), request.form.get('notes')))
        ...
        if form_name == 'hog_expense':
            execute('INSERT INTO hog_expenses(hog_cycle_id,entry_date,category,amount,description) VALUES(?,?,?,?,?)',
                    (int(request.form['hog_cycle_id']), request.form.get('entry_date'), request.form.get('category'), float(request.form.get('amount') or 0), request.form.get('description')))
    ...
    if request.method == 'POST':
        ...
        if form_name == 'fish_tx':
            ...
            fish_cycle_id = execute('INSERT INTO fish_cycles(cycle_id,period_name,start_date,status,notes) SELECT ?, ?, ?, "ACTIVE", ? WHERE NOT EXISTS(SELECT 1 FROM fish_cycles WHERE cycle_id=? AND status="ACTIVE")',
                                    (cycle['id'] if cycle else None, request.form.get('period_name') or 'Default Period', request.form.get('entry_date'), request.form.get('notes'), cycle['id'] if cycle else None)) if False else None
            ...
            if cycle and not active_fish_cycle:
                fc_id = execute('INSERT INTO fish_cycles(cycle_id,period_name,start_date,status,notes) VALUES(?,?,?,?,?)', (cycle['id'], request.form.get('period_name') or cycle['cycle_name'], request.form.get('entry_date'), 'ACTIVE', request.form.get('notes')))
            else:
