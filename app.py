...
def safe_date(value, default=None):
    value = (value or '').strip()
    return value or default or today_iso()


def latest_poultry_batch_id(cycle_id=None):
    if cycle_id:
        row = query('SELECT id FROM poultry_batches WHERE cycle_id=? ORDER BY id DESC LIMIT 1', (cycle_id,), one=True)
        if row:
            return row['id']
...
def latest_hog_cycle_id(cycle_id=None):
    if cycle_id:
        row = query('SELECT id FROM hog_cycles WHERE cycle_id=? ORDER BY id DESC LIMIT 1', (cycle_id,), one=True)
        if row:
            return row['id']
...
def latest_fish_cycle_id(cycle_id=None):
    if cycle_id:
        row = query('SELECT id FROM fish_cycles WHERE cycle_id=? ORDER BY id DESC LIMIT 1', (cycle_id,), one=True)
        if row:
            return row['id']
...
def unified_finance_history(module, active_only=True):
    ...
    if module == 'POULTRY':
        ...
        if cycle:
            ...
            sales = [r for r in sales if query('SELECT cycle_id FROM poultry_batches WHERE id=?', (r['batch_id'],), one=True)['cycle_id'] == cycle['id']]
            feed_logs = [r for r in feed_logs if query('SELECT cycle_id FROM poultry_batches WHERE id=?', (r['batch_id'],), one=True)['cycle_id'] == cycle['id']]
            expenses = [r for r in expenses if query('SELECT cycle_id FROM poultry_batches WHERE id=?', (r['batch_id'],), one=True)['cycle_id'] == cycle['id']]
    ...
    if request.method == 'POST':
        ...
        if form_name == 'batch':
            execute('INSERT INTO poultry_batches(cycle_id,poultry_type,batch_name,house_name,start_date,birds_count,supplier,cost,notes) VALUES(?,?,?,?,?,?,?,?,?)',
                    (cycle['id'] if cycle else None, safe_text(request.form.get('poultry_type'), 'Broiler'), safe_text(request.form.get('batch_name'), f'Batch {today_iso()}'), safe_text(request.form.get('house_name')), safe_date(request.form.get('start_date')), safe_int(request.form.get('birds_count')), safe_text(request.form.get('supplier')), safe_float(request.form.get('cost')), safe_text(request.form.get('notes'))))
            flash('Batch added.', 'success')
            return redirect(url_for('poultry_page', tab='batches'))
        if form_name == 'mortality':
            batch_id = safe_int(request.form.get('batch_id')) or latest_poultry_batch_id(cycle['id'] if cycle else None)
            if not batch_id:
                flash('Add a batch first before saving mortality.', 'danger')
                return redirect(url_for('poultry_page', tab='batches'))
            execute('INSERT INTO poultry_mortality(batch_id,entry_date,deaths,notes) VALUES(?,?,?,?)', (batch_id, safe_date(request.form.get('entry_date')), safe_int(request.form.get('deaths')), safe_text(request.form.get('notes'))))
            flash('Mortality saved.', 'success')
            return redirect(url_for('poultry_page', tab='operations'))
        if form_name == 'feed':
            batch_id = safe_int(request.form.get('batch_id')) or latest_poultry_batch_id(cycle['id'] if cycle else None)
            if not batch_id:
                flash('Add a batch first before saving feed usage.', 'danger')
                return redirect(url_for('poultry_page', tab='batches'))
            feed_id = execute('INSERT INTO poultry_feed_logs(batch_id,entry_date,feed_type,bags,amount,notes) VALUES(?,?,?,?,?,?)', (batch_id, safe_date(request.form.get('entry_date')), safe_text(request.form.get('feed_type'), 'Feed'), safe_float(request.form.get('bags')), safe_float(request.form.get('amount')), safe_text(request.form.get('notes'))))
        ...
        if form_name == 'feed_stock':
            ...
            total = sacks * cps
            inv_id = execute('INSERT INTO feed_inventory(module_name,entry_date,feed_type,sacks,cost_per_sack,total_cost,source_name,usage_type,house_name,cycle_id,notes) VALUES(?,?,?,?,?,?,?,?,?,?,?)', ('POULTRY', safe_date(request.form.get('entry_date')), safe_text(request.form.get('feed_type'), 'Feed'), sacks, cps, total, safe_text(request.form.get('source_name')), safe_text(request.form.get('usage_type'), 'Add to Inventory'), safe_text(request.form.get('house_name')), cycle['id'] if cycle else None, safe_text(request.form.get('notes'))))
        ...
        if form_name == 'sale':
            ...
            batch_id = safe_int(request.form.get('batch_id')) or latest_poultry_batch_id(cycle['id'] if cycle else None)
            if not batch_id:
                flash('Add a batch first before saving a sale.', 'danger')
                return redirect(url_for('poultry_page', tab='batches'))
            execute('INSERT INTO poultry_sales(batch_id,entry_date,buyer,birds_sold,kilos,price_per_kilo,total_amount,notes) VALUES(?,?,?,?,?,?,?,?)',
                    (batch_id, safe_date(request.form.get('entry_date')), safe_text(request.form.get('buyer'), 'Walk-in Buyer'), safe_int(request.form.get('birds_sold')), kilos, ppk, total, safe_text(request.form.get('notes'))))
        ...
        if form_name == 'expense':
            ...
            exp_id = execute(
                ...
                (
                    batch_id,
                    entry_date,
                    safe_text(request.form.get('category'), 'Other'),
                    safe_float(request.form.get('amount')),
                    safe_text(f"{safe_text(request.form.get('item_name'))}{' - ' + safe_text(request.form.get('description')) if safe_text(request.form.get('description')) else ''}", 'Expense'),
                    receipt_file
                )
            )
        ...
        if form_name == 'bulk_expense':
            ...
            batch_id = safe_int(request.form.get('batch_id')) or latest_poultry_batch_id(cycle['id'] if cycle else None)
            if not batch_id:
                flash('Add a batch first before saving bulk expenses.', 'danger')
                return redirect(url_for('poultry_page', tab='batches'))
            ...
            for item in rows:
                ...
                execute('INSERT INTO poultry_expenses(batch_id,entry_date,category,amount,description,group_ref,receipt_file) VALUES(?,?,?,?,?,?,?)', (batch_id, entry_date, item['category'], item['amount'], desc, group_ref, receipt_file))
...
def hog_page():
    ...
    if request.method == 'POST':
        ...
        if form_name == 'hog_bulk_expense':
            rows = parse_bulk_rows(request.form)
            group_ref = f"HB-{request.form.get('entry_date')}-{int(request.form.get('hog_cycle_id') or 0)}-{len(rows)}"
            receipt_file = save_uploaded_receipt(request.files.get('receipt_file'), group_ref.lower())
            for item in rows:
                execute('INSERT INTO hog_expenses(hog_cycle_id,entry_date,category,amount,description,group_ref,receipt_file) VALUES(?,?,?,?,?,?,?)', (int(request.form['hog_cycle_id']), request.form['entry_date'], item['category'], item['amount'], f"{item['description']} | Qty: {item['qty']} | Unit Cost: {item['unit_cost']}", group_ref, receipt_file))
    ...
    if request.method == 'POST':
        ...
        if form_name == 'fish_tx':
            ...
            execute('INSERT INTO fish_transactions(fish_cycle_id,entry_date,transaction_type,supplier,buyer,species,kilos,price_per_kilo,total_amount,notes) VALUES(?,?,?,?,?,?,?,?,?,?)',
                    (fc_id, request.form.get('entry_date'), request.form.get('transaction_type'), request.form.get('supplier'), request.form.get('buyer'), request.form.get('species'), kilos, ppk, total, request.form.get('notes')))
        ...
        if form_name == 'fish_expense':
            ...
            execute('INSERT INTO fish_expenses(fish_cycle_id,entry_date,category,amount,description,receipt_file) VALUES(?,?,?,?,?,?)',
                    (fc_id, request.form.get('entry_date'), request.form.get('category'), float(request.form.get('amount') or 0), request.form.get('description'), receipt_file))
        ...
        if form_name == 'fish_bulk_expense':
            ...
            fc_id = active_fish_cycle['id'] if active_fish_cycle else None
            rows = parse_bulk_rows(request.form)
            group_ref = f"FB-{request.form.get('entry_date')}-{int(fc_id or 0)}-{len(rows)}"
            receipt_file = save_uploaded_receipt(request.files.get('receipt_file'), group_ref.lower())
            for item in rows:
                execute('INSERT INTO fish_expenses(fish_cycle_id,entry_date,category,amount,description,group_ref,receipt_file) VALUES(?,?,?,?,?,?,?)', (fc_id, request.form['entry_date'], item['category'], item['amount'], f"{item['description']} | Qty: {item['qty']} | Unit Cost: {item['unit_cost']}", group_ref, receipt_file))
...
def handle_sqlite_operational_error(err):
    message = str(err).lower()
    if 'locked' in message:
        flash('Database is busy right now. Please retry once in a few seconds.', 'danger')
    else:
...
def handle_unexpected_error(err):
    if isinstance(err, sqlite3.OperationalError):
        raise err
    app.logger.exception('Unhandled application error: %s', err)
