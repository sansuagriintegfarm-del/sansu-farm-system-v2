...
def active_participants():
    return query('SELECT * FROM participants WHERE active=1 ORDER BY name')

def linked_participants(cycle):
    if not cycle:
        return []
    return query('''
...
def poultry_page():
    ...
    if request.method == 'POST':
        ...
        if form_name == 'expense':
            ...
            latest_batch = query('SELECT id FROM poultry_batches WHERE cycle_id=? AND status="ACTIVE" ORDER BY id DESC LIMIT 1', (cycle['id'],), one=True) if cycle else None
            exp_id = execute('INSERT INTO poultry_expenses(batch_id,entry_date,category,amount,description,receipt_file) VALUES(?,?,?,?,?,?)',
                (latest_batch['id'] if latest_batch else None, request.form['entry_date'], request.form.get('category'), float(request.form.get('amount') or 0), f"{request.form.get('item_name')}{' - ' + request.form.get('description') if request.form.get('description') else ''}", receipt_file))
