...
def normalize_sql(sql):
    ...
    sql = re.sub(r'ORDER BY\s+name\s*,', 'ORDER BY LOWER(name),', sql, flags=re.IGNORECASE)
    sql = re.sub(r'ORDER BY\s+p\.name\s*,', 'ORDER BY LOWER(p.name),', sql, flags=re.IGNORECASE)
    sql = re.sub(r'"([^"]*)"', lambda m: "'" + m.group(1) + "'", sql)
    sql = sql.replace('?', '%s')
    return sql
...
def query(sql, params=(), one=False, commit=False):
    db = get_db()
    cur = db.execute(normalize_sql(sql), params)
    rows = cur.fetchall()
    cur.close()
    if commit:
        db.commit()
...
def execute(sql, params=()):
    ...
    if sql_norm.strip().upper().startswith('INSERT ') and ' RETURNING ' not in sql_norm.upper():
        sql_norm += ' RETURNING id'
    cur = db.execute(sql_norm, params)
    last_id = None
    try:
        row = cur.fetchone()
        if row:
    ...
    with db:
        with db.cursor() as cur:
            ...
            if 'admin' not in existing_users:
                cur.execute('INSERT INTO users(username,password,full_name,role) VALUES(%s,%s,%s,%s)', ('admin', generate_password_hash('admin123'), 'Administrator', 'Admin'))
            if 'secretary' not in existing_users:
                cur.execute('INSERT INTO users(username,password,full_name,role) VALUES(%s,%s,%s,%s)', ('secretary', generate_password_hash('secretary123'), 'Farm Secretary', 'Secretary'))
    db.close()
else:
    ...
    if USE_POSTGRES:
        ...
        with db:
            with db.cursor() as cur:
                ...
                if not exists:
                    cur.execute(f'ALTER TABLE {table} ADD COLUMN {column} {normalize_schema_sql(definition)}')
        db.close()
    else:
        db = sqlite3.connect(DB_PATH)
