...
HOG_TYPES = ['Piglets', 'Sows', 'Fattener', 'Boar', 'Other']
...
def participant_allocations(cycle, finance):
    if not cycle:
        return []
    parts, share = participant_shares(cycle['id'], max(float(finance.get('profit') or 0), 0.0))
    allocations = []
    for p in parts:
        allocations.append({
            'id': p['id'],
...
def owner_income_share(module_name, finance, cycle=None):
    ...
    if owner_count <= 0:
        return 0.0
    return round(max(share_per_participant * owner_count, 0.0), 2)
...
def participant_display_rows(module_name, cycle, finance):
    ...
    share = (profit / count) if count else 0.0
    display_rows = []
    for p in participant_rows:
        item = dict(p)
        item['income_share'] = round(max(share, 0.0), 2)
        display_rows.append(item)
    return display_rows, round(max(share, 0.0), 2)
...
def poultry_house_monitoring():
    ...
    for row in houses.values():
        ...
        row['income_share'] = round((max(row['profit'], 0) / participant_count), 2) if participant_count else 0
...
def dashboard_context():
    ...
    hog_rows = query('SELECT pen_name, hog_type, heads FROM hog_cycles WHERE status="ACTIVE"')
    ...
    for row in hog_rows:
        hog_type = clean_text(row_value(row, 'hog_type', row_value(row, 'pen_name', ''))).lower()
    ...
    poultry_cashflow_rows, poultry_cashflow_summary = module_cashflow('POULTRY', active_only=True)
    hog_cashflow_rows, hog_cashflow_summary = module_cashflow('HOG', active_only=True)
    fish_cashflow_rows, fish_cashflow_summary = module_cashflow('FISH', active_only=True)
    modules = {'POULTRY': poultry_active, 'HOG': hog_active, 'FISH': fish_active}
    cashflow_modules = {
        'POULTRY': poultry_cashflow_summary,
        'HOG': hog_cashflow_summary,
        'FISH': fish_cashflow_summary,
    }
    total_remaining = max(sum(max(float(v.get('remaining', 0) or 0), 0) for v in cashflow_modules.values()), 1)
    module_mix = {k.lower(): safe_pct(max(float(v.get('remaining', 0) or 0), 0), total_remaining) for k, v in cashflow_modules.items()}
    ...
    cashflow_chart = {k.lower(): float(v.get('remaining', 0) or 0) for k, v in cashflow_modules.items()}
...
def table_has_column(table, column):
    if USE_POSTGRES:
        db = _connect_db()
        try:
            sql = "SELECT 1 FROM information_schema.columns WHERE table_schema='public' AND table_name=%s AND column_name=%s"
            with db.cursor() as cur:
...
def migrate_db():
    ...
    db = _connect_db()
    try:
        update_sql, update_params = _adapt_sql("UPDATE participants SET module_name='POULTRY' WHERE module_name IS NULL OR TRIM(module_name)=''", ())
        owner_sql = normalize_schema_sql('''
        CREATE TABLE IF NOT EXISTS owner_withdrawals(
        ...
        ''')
        owner_sql, owner_params = _adapt_sql(owner_sql, ())
        if USE_POSTGRES:
            with db.cursor() as cur:
                cur.execute(update_sql, update_params)
                cur.execute(owner_sql, owner_params)
            db.commit()
        else:
            db.execute(update_sql, update_params)
            db.execute(owner_sql, owner_params)
            db.commit()
    finally:
        db.close()
    ...
    if request.method == 'POST':
        ...
        if form_name == 'bulk_expense':
            ...
            for item in rows:
                ...
                total_bulk_amount += safe_float(item['amount'])
            ...
            if request.form.get('account_id') and total_bulk_amount > 0:
                sync_bank_balance(request.form.get('account_id'), total_bulk_amount, 'POULTRY', request.form['entry_date'], 'WITHDRAWAL', purpose='Poultry bulk expense', notes=f'Bulk expense {group_ref}')
    ...
    if request.method == 'POST':
        ...
        if form_name == 'hog_cycle':
            execute('INSERT INTO hog_cycles(cycle_id,pen_name,start_date,heads,source,cost,notes,hog_type) VALUES(?,?,?,?,?,?,?,?)',
                    (cycle['id'] if cycle else None, request.form.get('pen_name'), request.form.get('start_date'), int(request.form.get('heads') or 0), request.form.get('source'), float(request.form.get('cost') or 0), request.form.get('notes'), clean_text(request.form.get('hog_type'), 'Other')))
        ...
        if form_name == 'hog_bulk_expense':
            ...
            if request.form.get('account_id') and total_bulk_amount > 0:
                sync_bank_balance(request.form.get('account_id'), total_bulk_amount, 'HOG', entry_date, 'WITHDRAWAL', purpose='Hog bulk expense', notes=f'Bulk expense {group_ref}')
...
def fish_page():
    ...
    if request.method == 'POST':
        ...
        if form_name == 'fish_bulk_expense':
            ...
            if request.form.get('account_id') and total_bulk_amount > 0:
                sync_bank_balance(request.form.get('account_id'), total_bulk_amount, 'FISH', entry_date, 'WITHDRAWAL', purpose='Fish bulk expense', notes=f'Bulk expense {group_ref}')
...
def record_edit(record_type, record_id):
    ...
    return render_template('record_edit.html', record=row, record_type=record_type, fields=cfg['fields'], expense_categories=EXPENSE_CATEGORIES, tx_types=TX_TYPES, hog_types=HOG_TYPES)
