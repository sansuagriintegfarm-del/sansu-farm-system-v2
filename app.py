...
HOG_TYPES = ['Piglets', 'Sows', 'Fattener', 'Other']
...
def finance_summary_for_module(module, active_only=False):
    capital = query('SELECT COALESCE(SUM(amount),0) v FROM capital_entries WHERE module_name=?', (module,), one=True)['v']
    if module == 'POULTRY':
        ...
        if cycle:
            revenue = query('SELECT COALESCE(SUM(CASE WHEN ft.transaction_type="SELL" THEN ft.total_amount ELSE 0 END),0) v FROM fish_transactions ft JOIN fish_cycles fc ON fc.id=ft.fish_cycle_id WHERE fc.cycle_id=?', (cycle['id'],), one=True)['v']
            expenses = query('SELECT COALESCE(SUM(CASE WHEN ft.transaction_type IN ("BUY","EXPORT") THEN ft.total_amount ELSE 0 END),0) v FROM fish_transactions ft JOIN fish_cycles fc ON fc.id=ft.fish_cycle_id WHERE fc.cycle_id=?', (cycle['id'],), one=True)['v']
            expenses += query('SELECT COALESCE(SUM(fe.amount),0) v FROM fish_expenses fe JOIN fish_cycles fc ON fc.id=fe.fish_cycle_id WHERE fc.cycle_id=?', (cycle['id'],), one=True)['v']
        else:
            ...
            expenses = query('SELECT COALESCE(SUM(CASE WHEN transaction_type IN ("BUY","EXPORT") THEN total_amount ELSE 0 END),0) v FROM fish_transactions', one=True)['v']
...
def participant_allocations(cycle, finance):
    if not cycle:
        return []
    participants, share = participant_shares(cycle['id'], float(finance.get('profit') or 0))
    allocations = []
...
def dashboard_context():
    ...
    if poultry_cycle:
        poultry_live = query('SELECT COALESCE(SUM(birds_count),0) v FROM poultry_batches WHERE cycle_id=? AND status="ACTIVE"', (poultry_cycle['id'],), one=True)['v']
        poultry_live -= float(query('SELECT COALESCE(SUM(pm.deaths),0) v FROM poultry_mortality pm JOIN poultry_batches pb ON pb.id=pm.batch_id WHERE pb.cycle_id=?', (poultry_cycle['id'],), one=True)['v'] or 0)
        poultry_live -= float(query('SELECT COALESCE(SUM(ps.birds_sold),0) v FROM poultry_sales ps JOIN poultry_batches pb ON pb.id=ps.batch_id WHERE pb.cycle_id=?', (poultry_cycle['id'],), one=True)['v'] or 0)
    else:
        poultry_live = 0

    hog_breakdown = {'piglets': 0, 'sows': 0, 'fattener': 0, 'other': 0}
    if hog_cycle:
        for row in query('SELECT hog_type, heads FROM hog_cycles WHERE cycle_id=? AND status="ACTIVE"', (hog_cycle['id'],)):
            key = (row['hog_type'] or 'Other').strip().lower()
            if key not in hog_breakdown:
                key = 'other'
            hog_breakdown[key] += float(row['heads'] or 0)
        hog_sold = float(query('SELECT COALESCE(SUM(hs.heads),0) v FROM hog_sales hs JOIN hog_cycles hc ON hc.id=hs.hog_cycle_id WHERE hc.cycle_id=?', (hog_cycle['id'],), one=True)['v'] or 0)
    else:
        hog_sold = 0
    hog_heads = max(0, sum(hog_breakdown.values()) - hog_sold)

    if fish_cycle:
        fish_kilos = query('SELECT COALESCE(SUM(CASE WHEN ft.transaction_type="BUY" THEN ft.kilos ELSE -ft.kilos END),0) v FROM fish_transactions ft JOIN fish_cycles fc ON fc.id=ft.fish_cycle_id WHERE fc.cycle_id=?', (fish_cycle['id'],), one=True)['v']
        fish_buy = float(query('SELECT COALESCE(SUM(ft.kilos),0) v FROM fish_transactions ft JOIN fish_cycles fc ON fc.id=ft.fish_cycle_id WHERE fc.cycle_id=? AND ft.transaction_type="BUY"', (fish_cycle['id'],), one=True)['v'] or 0)
        fish_export = float(query('SELECT COALESCE(SUM(ft.kilos),0) v FROM fish_transactions ft JOIN fish_cycles fc ON fc.id=ft.fish_cycle_id WHERE fc.cycle_id=? AND ft.transaction_type="EXPORT"', (fish_cycle['id'],), one=True)['v'] or 0)
        fish_sales = float(query('SELECT COALESCE(SUM(ft.kilos),0) v FROM fish_transactions ft JOIN fish_cycles fc ON fc.id=ft.fish_cycle_id WHERE fc.cycle_id=? AND ft.transaction_type="SELL"', (fish_cycle['id'],), one=True)['v'] or 0)
    else:
        fish_kilos = fish_buy = fish_export = fish_sales = 0

    modules = {'POULTRY': poultry_active, 'HOG': hog_active, 'FISH': fish_active}
    module_mix = {k.lower(): round(max(v['remaining'], 0), 2) for k, v in modules.items()}
    ...
    return {
        ...
        'poultry_live': max(0, poultry_live or 0),
...
def capital_add():
    if not can_access_module(request.form.get('module_name')):
        ...
        return redirect(url_for('poultry_page'))
    amount = float(request.form.get('amount') or 0)
    account_id = request.form.get('destination_account_id') or None
    capital_id = execute('INSERT INTO capital_entries(cycle_id,module_name,entry_date,source_name,amount,destination_account_id,notes) VALUES(?,?,?,?,?,?,?)',
            (request.form.get('cycle_id') or None, request.form['module_name'], request.form['entry_date'], request.form.get('source_name'), amount, account_id, request.form.get('notes')))
    if account_id:
        execute('INSERT INTO bank_transactions(entry_date,account_id,module_name,tx_type,amount,reference_no,purpose,notes) VALUES(?,?,?,?,?,?,?,?)',
                (request.form['entry_date'], account_id, request.form['module_name'], 'DEPOSIT', amount, f'CAP-{capital_id}', request.form.get('source_name') or 'Capital entry', request.form.get('notes')))
        account = query('SELECT * FROM bank_accounts WHERE id=?', (account_id,), one=True)
        if account:
            execute('UPDATE bank_accounts SET current_balance=? WHERE id=?', (float(account['current_balance'] or 0) + amount, account_id))
    log_audit('ADD', 'capital', capital_id, f"{request.form['module_name']} capital {amount}")
    flash('Capital entry added.', 'success')
    return redirect(request.referrer or url_for('finance_page'))


def close_module_children(module, cycle_id):
    if module == 'POULTRY':
        execute('UPDATE poultry_batches SET status="ENDED" WHERE cycle_id=? AND status="ACTIVE"', (cycle_id,))
    elif module == 'HOG':
        execute('UPDATE hog_cycles SET status="ENDED" WHERE cycle_id=? AND status="ACTIVE"', (cycle_id,))
    elif module == 'FISH':
        execute('UPDATE fish_cycles SET status="ENDED" WHERE cycle_id=? AND status="ACTIVE"', (cycle_id,))


def start_cycle(module, cycle_name, poultry_type=None, notes=None, participants=None):
    previous = query('SELECT id FROM cycles WHERE module_name=? AND status="ACTIVE" ORDER BY id DESC LIMIT 1', (module,), one=True)
    if previous:
        execute('UPDATE cycles SET status="ENDED", end_date=date("now") WHERE id=?', (previous['id'],))
        close_module_children(module, previous['id'])
...
def cycle_end(cycle_id):
    ...
    close_module_children(cycle['module_name'], cycle_id)
...
def poultry_page():
    ...
    if request.method == 'POST':
        ...
        if form_name == 'sale':
            ...
            if not batch_id and cycle:
                latest_batch = query('SELECT id FROM poultry_batches WHERE cycle_id=? AND status="ACTIVE" ORDER BY id DESC LIMIT 1', (cycle['id'],), one=True)
            ...
            execute('INSERT INTO poultry_sales(batch_id,entry_date,buyer,birds_sold,kilos,price_per_kilo,total_amount,notes) VALUES(?,?,?,?,?,?,?,?)',
                    (batch_id, request.form['entry_date'], request.form.get('buyer'), int(request.form.get('birds_sold') or 0), kilos, ppk, total, request.form.get('notes')))
        ...
        if form_name == 'expense':
            ...
            exp_id = execute(
                ...
                (
                    ((query('SELECT id FROM poultry_batches WHERE cycle_id=? AND status="ACTIVE" ORDER BY id DESC LIMIT 1', (cycle['id'],), one=True)['id']) if cycle and query('SELECT id FROM poultry_batches WHERE cycle_id=? AND status="ACTIVE" ORDER BY id DESC LIMIT 1', (cycle['id'],), one=True) else None),
...
def hog_page():
    ...
    if request.method == 'POST':
        ...
        if form_name == 'hog_cycle':
            execute('INSERT INTO hog_cycles(cycle_id,pen_name,start_date,heads,source,hog_type,cost,notes) VALUES(?,?,?,?,?,?,?,?)',
                    (cycle['id'] if cycle else None, request.form.get('pen_name'), request.form.get('start_date'), int(request.form.get('heads') or 0), request.form.get('source'), request.form.get('hog_type') or 'Other', float(request.form.get('cost') or 0), request.form.get('notes')))
    ...
    visuals = module_visuals(finance, cashflow_summary)
    allocations = participant_allocations(cycle, finance)
    return render_template('hog.html', tab=tab, cycle=cycle, cycle_history=cycle_history('HOG'), cycles=cycles, feed_logs=feed_logs, sales=sales, expenses=expenses, capital=capital, finance=finance, finance_history=finance_history, participants=query('SELECT * FROM participants WHERE active=1'), share=(allocations[0]['income_share'] if allocations else 0), share_participants=allocations, participant_allocations=allocations, expense_categories=EXPENSE_CATEGORIES, bank_accounts=query('SELECT * FROM bank_accounts ORDER BY account_name'), cashflow_rows=cashflow_rows, cashflow_summary=cashflow_summary, visuals=visuals, hog_types=HOG_TYPES)
...
def get_edit_config(record_type):
    configs = {
        ...
        'hog_cycle': {'table':'hog_cycles','fields':['pen_name','start_date','heads','source','hog_type','cost','status','notes'],'redirect':'hog_page','tab':'operations'},
