...
def owner_income_share(module_name, finance, cycle=None):
    cycle = cycle or get_cycle(module_name)
    if not cycle:
        return 0.0
    parts, share = participant_shares(cycle['id'], float((finance or {}).get('profit') or 0))
    owner_count = sum(1 for p in parts if (p['role'] or '').strip().lower() == 'owner')
    if owner_count <= 0:
        return 0.0
    return round(share * owner_count, 2)


def owner_withdrawal_summary(owner_share_total):
    owner_share_total = round(float(owner_share_total or 0), 2)
    withdrawable_base = round(owner_share_total * 0.5, 2)
    owner_withdrawn_total = round(float(query(
        'SELECT COALESCE(SUM(amount),0) v FROM bank_transactions WHERE tx_type="WITHDRAWAL" AND module_name="OWNER"',
        one=True
    )['v'] or 0), 2)
    withdrawable_remaining = round(max(withdrawable_base - owner_withdrawn_total, 0), 2)
    return {
        'owner_share_total': owner_share_total,
        'withdrawable_base': withdrawable_base,
        'owner_withdrawn_total': owner_withdrawn_total,
...
def dashboard_context():
    ...
    owner_financials = owner_withdrawal_summary(owner_share_total)
    return {
        ...
        'owner_share_total': owner_financials['owner_share_total'],
        'withdrawable_base': owner_financials['withdrawable_base'],
        'owner_withdrawn_total': owner_financials['owner_withdrawn_total'],
        'withdrawable_remaining': owner_financials['withdrawable_remaining'],
        ...
        'bank_accounts': query('SELECT * FROM bank_accounts ORDER BY account_name COLLATE NOCASE'),
...
def owner_withdraw():
    ...
    amount = round(safe_float(request.form.get('amount')), 2)
    entry_date = safe_date(request.form.get('entry_date'), date.today().isoformat())
    reference_no = clean_text(request.form.get('reference_no'))
    notes = clean_text(request.form.get('notes'))

    if not account_id:
        flash('Select a bank account for owner withdrawal.', 'danger')
        return redirect(url_for('dashboard'))
    if amount <= 0:
        flash('Enter a valid withdrawal amount.', 'danger')
        return redirect(url_for('dashboard'))

    owner_financials = owner_withdrawal_summary(dashboard_context()['owner_share_total'])
    if amount > owner_financials['withdrawable_remaining']:
        flash(f'Withdrawal exceeds available owner profit of P{owner_financials["withdrawable_remaining"]:,.2f}.', 'danger')
        return redirect(url_for('dashboard'))
    ...
    if not account:
        ...
        return redirect(url_for('dashboard'))
    current_balance = round(float(account['current_balance'] or 0), 2)
    if amount > current_balance:
        flash(f'Withdrawal exceeds bank balance of P{current_balance:,.2f}.', 'danger')
        return redirect(url_for('dashboard'))
    ...
    execute('UPDATE bank_accounts SET current_balance=? WHERE id=?', (round(current_balance - amount, 2), account_id))
    log_audit('ADD', 'owner_withdrawal', account_id, f'Owner withdrawal {amount}')
    flash('Owner withdrawal saved and bank balance updated.', 'success')
    return redirect(url_for('dashboard'))
