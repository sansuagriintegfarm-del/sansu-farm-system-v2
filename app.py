...
def row_dict(row):
    ...
    try:
        ...
        out = {}
        for key in ("id", "account_name", "bank_name", "account_type", "opening_balance", "current_balance", "notes"):
            out[key] = row_value(row, key)
        return out


def list_bank_accounts():
    try:
        rows = query("SELECT id, COALESCE(account_name, '') AS account_name, COALESCE(bank_name, '') AS bank_name, COALESCE(account_type, '') AS account_type, COALESCE(opening_balance, 0) AS opening_balance, COALESCE(current_balance, 0) AS current_balance, COALESCE(notes, '') AS notes FROM bank_accounts ORDER BY COALESCE(account_name, ''), COALESCE(bank_name, ''), id")
...
def total_bank_balance():
    try:
        row = query('SELECT COALESCE(SUM(current_balance),0) v FROM bank_accounts', one=True)
        return safe_float(row_value(row, 'v', 0), 0)
    except Exception:
        return 0.0
...
def bank_option_label(account):
    ...
    head = account_name or bank_name or 'Bank Account'
    detail = ' · '.join(x for x in [bank_name, account_type] if x and x != head)
    suffix = f" — {detail}" if detail else ''
    return f"{head}{suffix} ({current_balance:,.2f})"


def recompute_all_bank_balances():
    accounts = list_bank_accounts()
    for account in accounts:
        account_id = row_value(account, 'id')
        opening = safe_float(row_value(account, 'opening_balance', 0))
        tx_row = query('SELECT COALESCE(SUM(CASE WHEN tx_type IN ("DEPOSIT","TRANSFER IN") THEN amount ELSE -amount END),0) v FROM bank_transactions WHERE account_id=?', (account_id,), one=True)
        movement = safe_float(row_value(tx_row, 'v', 0))
        execute('UPDATE bank_accounts SET current_balance=? WHERE id=?', (opening + movement, account_id))
...
def dashboard_context():
    ...
    hog_rows = query('SELECT hog_type, pen_name, heads FROM hog_cycles WHERE status="ACTIVE"')
    ...
    for row in hog_rows:
        hog_type = clean_text(row_value(row, 'hog_type') or row_value(row, 'pen_name')).lower()
        heads = float(row_value(row, 'heads', 0) or 0)
...
def bank_page():
    if request.method == 'POST' and request.form.get('form_name') == 'account':
        ...
        execute('INSERT INTO bank_accounts(account_name,bank_name,account_type,opening_balance,current_balance,notes) VALUES(?,?,?,?,?,?)',
                (clean_text(request.form.get('account_name')), clean_text(request.form.get('bank_name')), clean_text(request.form.get('account_type'), 'Bank'), opening, opening, clean_text(request.form.get('notes'))))
        flash('Account added.', 'success')
        return redirect(url_for('bank_page'))
    accounts = list_bank_accounts()[::-1]
