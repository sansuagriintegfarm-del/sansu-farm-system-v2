...
def normalize_schema_sql(schema_sql: str) -> str:
    if not USE_POSTGRES:
        return schema_sql
    normalized = schema_sql.replace('AUTOINCREMENT', '')
    normalized = re.sub(r'\bINTEGER PRIMARY KEY\b', 'SERIAL PRIMARY KEY', normalized)
    return normalized
...
def _adapt_sql(sql, params=()):
    if not USE_POSTGRES:
        return sql, params
    sql = re.sub(r'date\("now"\)', "TO_CHAR(CURRENT_DATE, 'YYYY-MM-DD')", sql, flags=re.IGNORECASE)
    sql = re.sub(r'\bCOLLATE\s+NOCASE\b', '', sql, flags=re.IGNORECASE)
    sql = re.sub(r'(?<![A-Za-z0-9_])"([^"\\]*(?:\\.[^"\\]*)*)"', lambda m: "'" + m.group(1).replace("'", "''") + "'", sql)
    sql = sql.replace('?', '%s')
    return sql, params
...
def execute(sql, params=()):
    ...
    sql, params = _adapt_sql(sql, params)
    if USE_POSTGRES:
        if ' returning ' not in sql.lower() and sql.lstrip().upper().startswith('INSERT'):
