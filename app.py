...
def participant_display_rows(module_name, cycle, finance):
    ...
    return display_rows, max(share, 0.0)
...
def dashboard_context():
    ...
    for row in hog_rows:
        hog_type = (row.get('hog_type') if isinstance(row, dict) else row['hog_type'] if 'hog_type' in row.keys() else None) or ''
        category = hog_type.strip().lower()
        pen = (row['pen_name'] or '').lower()
        heads = float(row['heads'] or 0)
        if category in ('piglet', 'piglets') or 'piglet' in pen:
            hog_breakdown['piglets'] += heads
        elif category in ('sow', 'sows', 'breeder') or 'sow' in pen or 'breeder' in pen:
            hog_breakdown['sows'] += heads
        elif category in ('fattener', 'finisher', 'grower') or 'fattener' in pen or 'finisher' in pen or 'grower' in pen:
            hog_breakdown['fattener'] += heads
        elif category in ('boar', 'boars') or 'boar' in pen:
...
def record_edit(record_type, record_id):
    ...
    if not row:
        ...
        return redirect(url_for('dashboard'))
    original_account_id = safe_int(row_value(row, 'account_id', 0))
    if request.method == 'POST':
        fields = []
        values = []
        for f in cfg['fields']:
            ...
            if f in ['birds_count', 'deaths', 'birds_sold', 'heads']:
                ...
                val = None if val in (None, '') else safe_int(val)
            elif f in ['cost', 'bags', 'amount', 'kilos', 'price_per_kilo', 'total_amount']:
                val = safe_float(val)
            else:
                val = clean_text(val) if val is not None else val
            fields.append(f'{f}=?')
            values.append(val)
        ...
        if record_type == 'bank_tx':
            new_account_id = safe_int(request.form.get('account_id'))
            if original_account_id:
                recompute_bank_balance(original_account_id)
            if new_account_id and new_account_id != original_account_id:
                recompute_bank_balance(new_account_id)
            elif new_account_id:
                recompute_bank_balance(new_account_id)
    ...
    return render_template('record_edit.html', record=row, record_type=record_type, fields=cfg['fields'], expense_categories=EXPENSE_CATEGORIES, tx_types=TX_TYPES, bank_accounts=list_bank_accounts())
...
def record_delete(record_type, record_id):
    ...
    if not row:
        flash('Record not found.', 'danger')
        return redirect(url_for('dashboard'))
    bank_account_id = safe_int(row_value(row, 'account_id', 0)) if record_type == 'bank_tx' else 0
    log_audit('DELETE', record_type, record_id, 'Record deleted')
    execute(f'DELETE FROM {cfg["table"]} WHERE id=?', (record_id,))
    if record_type == 'bank_tx' and bank_account_id:
        recompute_bank_balance(bank_account_id)
