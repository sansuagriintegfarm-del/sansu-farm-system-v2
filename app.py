...
def owner_income_share(module_name, finance, cycle=None):
    profit = float((finance or {}).get('profit') or 0)
    participants = participant_rows_for_profit(module_name, cycle or get_cycle(module_name))
    if not participants:
        return 0.0
    share_per_participant = profit / len(participants)
    owner_count = sum(1 for p in participants if (p.get('role') or '').strip().lower() == 'owner')
    if owner_count <= 0:
        return 0.0
    return round(share_per_participant * owner_count, 2)
...
def participant_display_rows(module_name, cycle, finance):
    profit = float(finance.get('profit') or 0)
...
def dashboard_context():
    ...
    return {
        ...
        'bank_accounts': query('SELECT * FROM bank_accounts ORDER BY account_name'),
...
def recompute_bank_balance(account_id):
    ...
    if not account:
        return 0.0
    opening_balance = safe_float(account.get('opening_balance'))
    tx_rows = query('SELECT tx_type, amount FROM bank_transactions WHERE account_id=? ORDER BY id', (account_id,))
    running = opening_balance
    for tx in tx_rows:
        amount = safe_float(tx.get('amount'))
        tx_type = clean_text(tx.get('tx_type')).upper()
        if tx_type in ('DEPOSIT', 'TRANSFER IN'):
            running += amount
        else:
            running -= amount
    execute('UPDATE bank_accounts SET current_balance=? WHERE id=?', (round(running, 2), account_id))
    return round(running, 2)


def sync_bank_transaction(source_record_type, source_record_id, account_id, amount, module_name, entry_date, tx_type, reference_no='', purpose='', notes=''):
    source_record_type = clean_text(source_record_type)
    source_record_id = safe_int(source_record_id)
    account_id = safe_int(account_id)
    amount = safe_float(amount)
    existing = query('SELECT * FROM bank_transactions WHERE source_record_type=? AND source_record_id=? ORDER BY id DESC LIMIT 1', (source_record_type, source_record_id), one=True)
    if existing:
        old_account_id = safe_int(existing.get('account_id'))
        if account_id and amount > 0:
            execute('UPDATE bank_transactions SET entry_date=?, account_id=?, module_name=?, tx_type=?, amount=?, reference_no=?, purpose=?, notes=? WHERE id=?',
                    (entry_date, account_id, module_name, tx_type, amount, clean_text(reference_no), clean_text(purpose), clean_text(notes), existing['id']))
            recompute_bank_balance(account_id)
            if old_account_id and old_account_id != account_id:
                recompute_bank_balance(old_account_id)
            return existing['id']
        execute('DELETE FROM bank_transactions WHERE id=?', (existing['id'],))
        if old_account_id:
            recompute_bank_balance(old_account_id)
        return None
    if not account_id or amount <= 0:
        return None
    tx_id = execute('INSERT INTO bank_transactions(entry_date,account_id,module_name,tx_type,amount,reference_no,purpose,notes,source_record_type,source_record_id) VALUES(?,?,?,?,?,?,?,?,?,?)',
                    (entry_date, account_id, module_name, tx_type, amount, clean_text(reference_no), clean_text(purpose), clean_text(notes), source_record_type, source_record_id))
    recompute_bank_balance(account_id)
    return tx_id


def purge_bank_transaction(source_record_type, source_record_id):
    return sync_bank_transaction(source_record_type, source_record_id, None, 0, '', '', '', '', '', '')


def refresh_bank_links_for_record(record_type, record_id):
    if record_type == 'capital':
        row = query('SELECT * FROM capital_entries WHERE id=?', (record_id,), one=True)
        if not row:
            return purge_bank_transaction('capital', record_id)
        return sync_bank_transaction('capital', record_id, row.get('destination_account_id'), row.get('amount'), row.get('module_name'), row.get('entry_date'), 'DEPOSIT', '', f"{clean_text(row.get('module_name'))} capital", clean_text(row.get('notes')))
    if record_type == 'poultry_sale':
        row = query('SELECT * FROM poultry_sales WHERE id=?', (record_id,), one=True)
        if not row:
            return purge_bank_transaction('poultry_sale', record_id)
        return sync_bank_transaction('poultry_sale', record_id, row.get('account_id'), row.get('total_amount'), 'POULTRY', row.get('entry_date'), 'DEPOSIT', '', 'Poultry sale', clean_text(row.get('notes')))
    if record_type == 'poultry_expense':
        row = query('SELECT * FROM poultry_expenses WHERE id=?', (record_id,), one=True)
        if not row:
            return purge_bank_transaction('poultry_expense', record_id)
        return sync_bank_transaction('poultry_expense', record_id, row.get('account_id'), row.get('amount'), 'POULTRY', row.get('entry_date'), 'WITHDRAWAL', '', clean_text(row.get('category'), 'Poultry expense'), clean_text(row.get('description')))
    if record_type == 'hog_sale':
        row = query('SELECT * FROM hog_sales WHERE id=?', (record_id,), one=True)
        if not row:
            return purge_bank_transaction('hog_sale', record_id)
        return sync_bank_transaction('hog_sale', record_id, row.get('account_id'), row.get('total_amount'), 'HOG', row.get('entry_date'), 'DEPOSIT', '', 'Hog sale', clean_text(row.get('notes')))
    if record_type == 'hog_expense':
        row = query('SELECT * FROM hog_expenses WHERE id=?', (record_id,), one=True)
        if not row:
            return purge_bank_transaction('hog_expense', record_id)
        return sync_bank_transaction('hog_expense', record_id, row.get('account_id'), row.get('amount'), 'HOG', row.get('entry_date'), 'WITHDRAWAL', '', clean_text(row.get('category'), 'Hog expense'), clean_text(row.get('description')))
    if record_type == 'fish_tx':
        row = query('SELECT * FROM fish_transactions WHERE id=?', (record_id,), one=True)
        if not row:
            return purge_bank_transaction('fish_tx', record_id)
        tx_kind = clean_text(row.get('transaction_type')).upper()
        bank_tx_type = 'WITHDRAWAL' if tx_kind == 'BUY' else 'DEPOSIT'
        return sync_bank_transaction('fish_tx', record_id, row.get('account_id'), row.get('total_amount'), 'FISH', row.get('entry_date'), bank_tx_type, '', f"Fish {tx_kind.lower()}", clean_text(row.get('notes')))
    if record_type == 'fish_expense':
        row = query('SELECT * FROM fish_expenses WHERE id=?', (record_id,), one=True)
        if not row:
            return purge_bank_transaction('fish_expense', record_id)
        return sync_bank_transaction('fish_expense', record_id, row.get('account_id'), row.get('amount'), 'FISH', row.get('entry_date'), 'WITHDRAWAL', '', clean_text(row.get('category'), 'Fish expense'), clean_text(row.get('description')))
    if record_type == 'owner_withdrawal':
        row = query('SELECT * FROM owner_withdrawals WHERE id=?', (record_id,), one=True)
        if not row:
            return purge_bank_transaction('owner_withdrawal', record_id)
        return sync_bank_transaction('owner_withdrawal', record_id, row.get('account_id'), row.get('amount'), 'OWNER', row.get('entry_date'), 'WITHDRAWAL', row.get('reference_no'), 'Owner profit withdrawal', clean_text(row.get('notes')))
...
def owner_withdraw():
    ...
    refresh_bank_links_for_record('owner_withdrawal', withdrawal_id)
...
def bank_tx_add():
    ...
    tx_id = execute('INSERT INTO bank_transactions(entry_date,account_id,module_name,tx_type,amount,reference_no,purpose,notes) VALUES(?,?,?,?,?,?,?,?)',
    ...
    recompute_bank_balance(account_id)
    log_audit('ADD', 'bank_tx', tx_id or account_id, f'{tx_type} {amount}')
...
def capital_add():
    ...
    refresh_bank_links_for_record('capital', capital_id)
    ...
    if request.method == 'POST':
        ...
        if form_name == 'sale':
            ...
            execute('INSERT INTO poultry_sales(batch_id,entry_date,buyer,birds_sold,kilos,price_per_kilo,total_amount,notes) VALUES(?,?,?,?,?,?,?,?)',
                    (int(request.form['batch_id']), request.form['entry_date'], request.form.get('buyer'), int(request.form.get('birds_sold') or 0), kilos, ppk, total, request.form.get('notes')))
        ...
        if form_name == 'bulk_expense':
            ...
            for item in rows:
                ...
                execute('INSERT INTO poultry_expenses(batch_id,entry_date,category,amount,description,group_ref,receipt_file) VALUES(?,?,?,?,?,?,?)', (int(request.form['batch_id']), request.form['entry_date'], item['category'], item['amount'], desc, group_ref, receipt_file))
...
def hog_page():
    ...
    if request.method == 'POST':
        ...
        if form_name == 'hog_sale':
            ...
            execute('INSERT INTO hog_sales(hog_cycle_id,entry_date,buyer,heads,kilos,total_amount,notes) VALUES(?,?,?,?,?,?,?)',
                    (hog_cycle_id, safe_date(request.form.get('entry_date')), clean_text(request.form.get('buyer')), safe_int(request.form.get('heads')), safe_float(request.form.get('kilos')), safe_float(request.form.get('total_amount')), clean_text(request.form.get('notes'))))
        ...
        if form_name == 'hog_expense':
            ...
            execute('INSERT INTO hog_expenses(hog_cycle_id,entry_date,category,amount,description,receipt_file,receipt_name) VALUES(?,?,?,?,?,?,?)',
                    (hog_cycle_id, safe_date(request.form.get('entry_date')), clean_text(request.form.get('category'),'Other'), safe_float(request.form.get('amount')), clean_text(request.form.get('description')), receipt_file, secure_filename(getattr(request.files.get('receipt_file'), 'filename', '') or '')))
        ...
        if form_name == 'hog_bulk_expense':
            ...
            for item in rows:
                execute('INSERT INTO hog_expenses(hog_cycle_id,entry_date,category,amount,description,group_ref,receipt_file,receipt_name) VALUES(?,?,?,?,?,?,?,?)', (hog_cycle_id, entry_date, item['category'], item['amount'], f"{item['description']} | Qty: {item['qty']} | Unit Cost: {item['unit_cost']}", group_ref, receipt_file, receipt_name))
    ...
    return render_template('hog.html', tab=tab, cycle=cycle, cycle_history=cycle_history('HOG'), cycles=cycles, feed_logs=feed_logs, sales=sales, expenses=expenses, capital=capital, finance=finance, finance_history=finance_history, participants=available_participants, available_participants=available_participants, share=share, share_participants=share_participants, expense_categories=EXPENSE_CATEGORIES, bank_accounts=query('SELECT * FROM bank_accounts ORDER BY account_name'), cashflow_rows=cashflow_rows, cashflow_summary=cashflow_summary, visuals=visuals)
    ...
    if request.method == 'POST':
        ...
        if form_name == 'fish_tx':
            ...
            execute('INSERT INTO fish_transactions(fish_cycle_id,entry_date,transaction_type,supplier,buyer,species,kilos,price_per_kilo,total_amount,notes) VALUES(?,?,?,?,?,?,?,?,?,?)',
                    (fc_id, safe_date(request.form.get('entry_date')), clean_text(request.form.get('transaction_type'),'BUY'), clean_text(request.form.get('supplier')), clean_text(request.form.get('buyer')), clean_text(request.form.get('species'),'Fish'), kilos, ppk, total, clean_text(request.form.get('notes'))))
        ...
        if form_name == 'fish_expense':
            ...
            execute('INSERT INTO fish_expenses(fish_cycle_id,entry_date,category,amount,description,receipt_file,receipt_name) VALUES(?,?,?,?,?,?,?)',
                    (fc_id, safe_date(request.form.get('entry_date')), clean_text(request.form.get('category'),'Other'), safe_float(request.form.get('amount')), clean_text(request.form.get('description')), receipt_file, secure_filename(getattr(request.files.get('receipt_file'), 'filename', '') or '')))
        ...
        if form_name == 'fish_bulk_expense':
            ...
            receipt_name = secure_filename(getattr(request.files.get('receipt_file'), 'filename', '') or '')
            for item in rows:
                execute('INSERT INTO fish_expenses(fish_cycle_id,entry_date,category,amount,description,group_ref,receipt_file,receipt_name) VALUES(?,?,?,?,?,?,?,?)', (fc_id, entry_date, item['category'], item['amount'], f"{item['description']} | Qty: {item['qty']} | Unit Cost: {item['unit_cost']}", group_ref, receipt_file, receipt_name))
...
def get_edit_config(record_type):
    configs = {
        ...
        'poultry_sale': {'table':'poultry_sales','fields':['entry_date','buyer','birds_sold','kilos','price_per_kilo','notes','account_id'],'redirect':'poultry_page','tab':'finance'},
        'poultry_expense': {'table':'poultry_expenses','fields':['entry_date','category','amount','description','account_id'],'redirect':'poultry_page','tab':'finance'},
        ...
        'hog_sale': {'table':'hog_sales','fields':['entry_date','buyer','heads','kilos','total_amount','notes','account_id'],'redirect':'hog_page','tab':'finance'},
        'hog_expense': {'table':'hog_expenses','fields':['entry_date','category','amount','description','account_id'],'redirect':'hog_page','tab':'finance'},
        'fish_tx': {'table':'fish_transactions','fields':['entry_date','transaction_type','supplier','buyer','species','kilos','price_per_kilo','notes','account_id'],'redirect':'fish_page','tab':'operations'},
        'fish_expense': {'table':'fish_expenses','fields':['entry_date','category','amount','description','account_id'],'redirect':'fish_page','tab':'finance'},
        'capital': {'table':'capital_entries','fields':['entry_date','source_name','amount','notes','destination_account_id'],'redirect':'finance_page','tab':None},
...
def record_edit(record_type, record_id):
    ...
    if request.method == 'POST':
        ...
        for f in cfg['fields']:
            ...
            if f in ['birds_count','deaths','birds_sold','heads']:
                val = int(val or 0)
        ...
        if record_type in {'capital','poultry_sale','poultry_expense','hog_sale','hog_expense','fish_tx','fish_expense'}:
            refresh_bank_links_for_record(record_type, record_id)
...
def record_delete(record_type, record_id):
    ...
    if record_type in {'capital','poultry_sale','poultry_expense','hog_sale','hog_expense','fish_tx','fish_expense','owner_withdrawal'}:
        purge_bank_transaction(record_type, record_id)
