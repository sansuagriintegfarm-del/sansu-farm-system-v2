...
def participant_display_rows(module_name, cycle, finance):
    if cycle:
        linked, share = participant_shares(cycle['id'], float(finance.get('profit') or 0))
        if linked:
            return linked, share
    module_rows = module_participants(module_name, active_only=True)
    count = len(module_rows)
    share = (float(finance.get('profit') or 0) / count) if count else 0
    display_rows = []
...
def migrate_db():
    ...
    execute("UPDATE participants SET module_name='POULTRY' WHERE module_name IS NULL OR TRIM(module_name)=''")
