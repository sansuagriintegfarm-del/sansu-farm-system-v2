...
def dashboard_context():
    ...
    for row in hog_rows:
        hog_type = clean_text(row_value(row, 'hog_type', ''), '').lower()
        pen = clean_text(row_value(row, 'pen_name', ''), '').lower()
        heads = float(row_value(row, 'heads', 0) or 0)
        source = hog_type or pen
        if 'piglet' in source:
            hog_breakdown['piglets'] += heads
        elif 'sow' in source or 'breeder' in source:
            hog_breakdown['sows'] += heads
        elif 'fattener' in source or 'finisher' in source or 'grower' in source:
...
def get_edit_config(record_type):
    configs = {
        ...
        'hog_cycle': {'table':'hog_cycles','fields':['pen_name','start_date','hog_type','heads','source','cost','status','notes'],'redirect':'hog_page','tab':'operations'},
