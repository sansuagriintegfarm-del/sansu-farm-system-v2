...
def today_iso():
    row = query('SELECT date("now") d', one=True)
...
def latest_poultry_batch(cycle_id=None):
    if cycle_id:
        row = query('SELECT * FROM poultry_batches WHERE cycle_id=? ORDER BY id DESC LIMIT 1', (cycle_id,), one=True)
        if row:
            return row
    return query('SELECT * FROM poultry_batches ORDER BY id DESC LIMIT 1', one=True)
...
def poultry_page():
    ...
    if request.method == 'POST':
        ...
        if form_name == 'batch':
            poultry_type = (request.form.get('poultry_type') or 'Broiler').strip() or 'Broiler'
            batch_name = (request.form.get('batch_name') or '').strip() or f"{poultry_type} Batch"
            house_name = (request.form.get('house_name') or '').strip() or 'Main House'
            start_date = (request.form.get('start_date') or '').strip() or today_iso()
            execute('INSERT INTO poultry_batches(cycle_id,poultry_type,batch_name,house_name,start_date,birds_count,supplier,cost,notes) VALUES(?,?,?,?,?,?,?,?,?)',
                    (cycle['id'] if cycle else None, poultry_type, batch_name, house_name, start_date, safe_int(request.form.get('birds_count')), request.form.get('supplier'), safe_float(request.form.get('cost')), request.form.get('notes')))
        ...
        if form_name == 'feed_stock':
            ...
            return redirect(url_for('poultry_page', tab='operations'))
        if form_name == 'sale':
            latest_batch = latest_poultry_batch(cycle['id'] if cycle else None)
            batch_id = safe_int(request.form.get('batch_id') or (latest_batch['id'] if latest_batch else 0))
            if not batch_id:
            ...
            execute('INSERT INTO poultry_sales(batch_id,entry_date,buyer,birds_sold,kilos,price_per_kilo,total_amount,notes) VALUES(?,?,?,?,?,?,?,?)',
                    (batch_id, (request.form.get('entry_date') or '').strip() or today_iso(), request.form.get('buyer'), safe_int(request.form.get('birds_sold')), kilos, ppk, total, request.form.get('notes')))
        ...
        if form_name == 'expense':
            ...
            batch_id = safe_int(request.form.get('batch_id') or (latest_batch['id'] if latest_batch else 0))
            if not batch_id:
                flash('Add a batch first before saving an expense.', 'danger')
                return redirect(url_for('poultry_page', tab='finance'))
            entry_date = (request.form.get('entry_date') or '').strip() or today_iso()
            ...
            full_desc = ' - '.join([part for part in [item_name or category, description] if part])
            exp_id = execute(
                ...
                (
                    ...
                    full_desc or category,
                    receipt_file
                )
            )
            log_audit('ADD', 'poultry_expense', exp_id, full_desc or category)
        ...
        if form_name == 'bulk_expense':
            ...
            if not rows:
                flash('No bulk expense rows to save.', 'warning')
                return redirect(url_for('poultry_page', tab='finance'))
            entry_date = (request.form.get('entry_date') or '').strip() or today_iso()
            group_ref = f"PB-{entry_date}-{batch_id}-{len(rows)}"
